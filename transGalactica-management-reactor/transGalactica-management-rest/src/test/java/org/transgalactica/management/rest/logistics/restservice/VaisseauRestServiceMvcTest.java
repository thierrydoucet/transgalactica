...
public class VaisseauRestServiceMvcTest extends AbstractMvcTest {
	...
	public void create() throws Exception {
		...
		mockMvc.perform(get("/vaisseaux/immatriculation creation").accept(APPLICATION_JSON)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.nombreDePassagers").value(100))
				.andExpect(jsonPath("$.autonomie").value(10000)).andExpect(jsonPath("$.vitesse").value(2000));
	}
	...
	public void search() throws Exception {
		mockMvc.perform(
				get("/vaisseaux/search?immatriculation=Fau%&modele=cargo%&intergalactique=true")
	...
	private static final String SEARCH_FAUCON_MILLENIUM = "<vaisseaux><vaisseaux><immatriculation>Faucon Millenium</immatriculation><localisationHangar>Alderaan</localisationHangar><modele>cargo YT-1300</modele></vaisseaux></vaisseaux>";
}
